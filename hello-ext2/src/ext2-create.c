...
void write_superblock(int fd) {
    ...
    superblock.s_blocks_per_group  = BLOCK_SIZE * BYTE_TO_BITS;
    superblock.s_frags_per_group   = BLOCK_SIZE * BYTE_TO_BITS;
...
void write_block_bitmap(int fd) {
    ...
    unsigned char bits[1024] = {0};

    for (int count = 0; count < 3; count++) {
        bits[count] = 0xFF;
        if (count == 2) bits[count] = 0x7F;
    }

    bits[127] = 0x80;

    for (int count = 128; count < 1024; count++) {
        bits[count] = 0xFF;
    }

    if (write(fd, bits, BLOCK_SIZE) != BLOCK_SIZE) {
        errno_exit("write");
    }
}
...
void write_inode_bitmap(int fd) {
    ...
    for (int count = 16; count < 1024; count++) {
        bits[count] = 0xFF;
    }

    if (write(fd, bits, 1024) != 1024) {
        errno_exit("write");
    }
}
...
void write_inode_table(int fd) {
	...
	struct ext2_inode hello_link_node = {0};
	hello_link_node.i_mode = EXT2_S_IFLNK
	                              | EXT2_S_IRUSR
	                              | EXT2_S_IWUSR
	                              | EXT2_S_IRGRP
	                              | EXT2_S_IROTH;
	hello_link_node.i_uid = 1000;
	hello_link_node.i_size = 11;
	hello_link_node.i_atime = current_time;
	hello_link_node.i_ctime = current_time;
	hello_link_node.i_mtime = current_time;
	hello_link_node.i_dtime = 0;
	hello_link_node.i_gid = 1000;
	hello_link_node.i_links_count = 1;
	hello_link_node.i_blocks = 0; /* These are oddly 512 blocks */

	char hello_world[] = {'h','e','l','l','o','-','w','o','r','l','d'};
    memcpy(hello_link_node.i_block, hello_world, sizeof(hello_world)/sizeof(char));
	write_inode(fd, HELLO_INO, &hello_link_node);
...
void write_root_dir_block(int fd) {
    ...
    struct ext2_dir_entry lost_n_found = {0};
    dir_entry_set(lost_n_found, LOST_AND_FOUND_INO, "lost+found");
    dir_entry_write(lost_n_found, fd);

    bytes_remaining -= lost_n_found.rec_len;
